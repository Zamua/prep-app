...
const TEST_NOW = '2026-03-14T15:00:00+00:00';
...
function seedAnonymousAtCap(storage: FakeCellStorage, questions: number): void {
  ...
  sql.exec(`INSERT INTO decks (name, display_name, created_at) VALUES ('full', 'Full deck', ?)`, TEST_NOW);
  ...
  for (let i = 0; i < questions; i++) {
    ...
    sql.exec('INSERT INTO cards (question_id, step, next_due) VALUES (?, 0, ?)', qid, TEST_NOW);
  }
}
...
beforeAll(async () => {
  ...
  const cookie = await mintCookie(new HmacSigner(secret!), ANON_ID, Math.floor(new Date(TEST_NOW).getTime() / 1000));
