// One migrated user cell's repositories over fake storage, on the parity clock.
import type { Clock, Random, UserRepos } from '../../app/ports.js';
import { FixedClock } from '../../runtime/adapters/clock.js';
import { SeededRandom, ParitySessionIds } from '../../runtime/adapters/random.js';
...
export const PARITY_NOW = new Date('2026-03-14T15:00:00Z');
...
export function cell(opts: { anonymous?: boolean; profile?: boolean; at?: Date } = {}): Cell {
  ...
  if (opts.profile !== false) {
    ...
    else repos.prefs.upsert(USER, { email: USER, displayName: 'Parity' });
  }
