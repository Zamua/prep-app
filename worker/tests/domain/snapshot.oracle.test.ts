// The snapshot JSON is prompt bytes: the free-tier stub keys its canned
// replies on the message it is sent, so a reordered or extra key is a
// different message and no reply comes back. The expectations below are the
// literal output of `json.MarshalIndent` over the Go `cardForTransform` and
// `deckForTransform` structs, which is what the model used to be shown.
...
describe('the transform snapshot marshals as Go did', () => {
