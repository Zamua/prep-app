...
// here and not the signed-in page. Everything else on the screen is
// the shared dashboard.
const STATUS = "Signed out. These are the decks saved on this device.";

// Cards this device holds that belong to no deck it holds (authored
...
const EMPTY_LOCAL = {
  ...
  headEm: "on this device",
  sub:
    "These cards are not in a deck this device holds a copy of. Study " +
    "them here, or sign in to sync them back.",
  ctaLabel: "Study on this device",
};
...
function render(region, overview) {
  ...
  frag.appendChild(preludeView(overview, {isNewUser: false, status: STATUS}));
  frag.appendChild(
  ...
  frag.appendChild(
    deckListView(overview, {
      ...
      actions: [{icon: "arrow-up-right", label: "Study on this device", href: OFFLINE_HREF}],
