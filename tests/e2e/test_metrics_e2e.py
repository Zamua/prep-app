"""`GET /metrics` on a running node.

`worker/tests/metrics.test.ts` gates the encoder. This one gates the
deployed route: that the scrape exists, carries the exposition media
type, is not itself counted, and that a request through the real entry
worker lands under the template it matched.

    PARITY_BASE_URL=http://127.0.0.1:8791 \
    PARITY_INTERNAL_TOKEN=parity-internal-token \
    .venv/bin/pytest tests/e2e/test_metrics_e2e.py -q
...
SEED_USER = "parity@example.com"
SEED_USER_NAME = "Parity"
...
# Process- and runtime-level series a cell cannot produce: it has no
# process of its own to report on. A name reappearing here means one
# came back by accident with the wrong meaning.
GONE = (
...
def base_url() -> str:
    ...
    if not url:
        pytest.skip(f"set {BASE_URL_ENV} to a running node")
    return url.rstrip("/")
...
def get(path: str, *, identified: bool = False) -> tuple[int, dict[str, str], str]:
    ...
    if identified:
        headers |= {
            ...
            "tailscale-user-name": SEED_USER_NAME,
            "x-internal-token": os.environ.get(INTERNAL_TOKEN_ENV) or INTERNAL_TOKEN,
        }
