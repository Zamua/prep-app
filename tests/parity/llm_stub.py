"""Canned OpenAI-compatible chat-completions server for the parity gate.
...
Standalone:
    ...
    python -m tests.parity.llm_stub --port 8089 --fixtures <dir> [--record]
...
FIXTURES_DIR = Path(__file__).resolve().parents[1] / "fixtures" / "parity" / "llm"
MISSING_SUBDIR = "missing"

SERVER_HEADER = "parity-llm-stub"
DATE_HEADER = "Sat, 14 Mar 2026 15:00:00 GMT"  # PARITY_NOW
...
class LLMStub:
    ...
    def start(self) -> LLMStub:
        self._thread = threading.Thread(
            target=self._server.serve_forever, name="parity-llm-stub", daemon=True
        )
        self._thread.start()
        return self
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        parser.error(str(e))
    print(f"parity llm stub: {stub.base_url} fixtures={args.fixtures} record={record}", flush=True)
    stub.start()
    try:
        while True:
            time.sleep(3600)
