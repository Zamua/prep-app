...
every migrated card is scheduled twice - `migrate.legacy_srs` from the snapshot
row, `worker/domain/fsrs` from the cell row - and the two answers are
compared.
...
@contextmanager
def _no_fuzz():
    """py-fsrs fuzzes by default. Swapping the scheduler cache for one
    holding fuzz-disabled schedulers keeps `schedule_review` itself in the
    path, so the clamp, the rounding and the wrapper are still under
    test."""
    from fsrs import Scheduler

    from migrate import legacy_srs as srs
...
class PyFsrsOracle:
    """`migrate.legacy_srs.schedule_review`, the reference the port was
    written against."""

    def schedule(self, cards: Sequence[ScheduleInput], now: str) -> dict[str, dict[str, dict]]:
        from migrate.legacy_srs import CardSRSState, Verdict, schedule_review
