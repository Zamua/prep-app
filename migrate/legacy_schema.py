"""The pre-cutover SQLite schema, frozen.

A snapshot is the only reader of this shape now: `build(path)`
materialises it so `migrate.synth` can generate a prod-shaped file to
export, and the exporter reads real snapshots that were written by the
same bootstrap.

Nothing here is a live schema. Editing it changes what a synthetic
snapshot looks like, never what any deployed store holds.
"""
...
from migrate import clock

# Set for the duration of a `build()`; a read outside one is a bug.
DB_PATH: Path | None = None


def build(path: Path) -> None:
    """Materialise the frozen schema at `path`."""
    global DB_PATH
    previous = DB_PATH
    DB_PATH = Path(path)
    try:
...
def init() -> None:
    """Schema bootstrap plus every migration step, in order.

    CREATE TABLE IF NOT EXISTS for the initial shape, then one ALTER
    TABLE / table-rebuild block per step, each guarded by a `PRAGMA
    table_info` check so re-running is a no-op. Call it through
    `build()`, which points `DB_PATH` at the file.
    """
    with cursor() as c:
        ...
        # 6. UI-created decks need a free-form context prompt - what the
        #    model sees when generating cards. Fills the role that
        #    DECK_CONTEXT[*] used to play in source code. Existing rows
        #    have NULL until re-described.
        ...
        # 9. Trivia card explanations: a short paragraph the model
        #    generates alongside the Q+A. Surfaced in the trivia card
        #    view as a "Deep dive" disclosure so the user can learn the
        #    why behind the answer, not just memorize it. NULL for older
        #    trivia cards generated before this column existed; UI hides
        #    the section when null.
        ...
        if "answer_regex" not in qcols:
            ...
            # before falling back to the AI grader. Model-generated
            # alongside the Q+A; evolves over time when the user
            # re-grades and the model decides the user's answer is a
            # legitimate alternative form (synonym / abbreviation /
            # equivalent expression). Stored as a Python re-flavored
            # pattern; runtime applies re.IGNORECASE. NULL means "no
            # regex available" → grader falls through to the existing
