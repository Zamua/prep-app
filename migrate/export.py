"""The exporter: a pure read of a snapshot into a diffable directory.
    ...
    python -m migrate.export --snapshot <path> --out <dir>

Nothing is written outside `--out`, and the snapshot is opened read-only
and immutable (`migrate.snapshot`). This module never imports
`migrate.legacy_schema`, whose `init()` would run 27 migrations against
