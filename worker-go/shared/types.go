...
const (
	...
	// Query handler on TriviaGenerateWorkflow.
...
//
// Or terminates early at `failed` (claude error / no cards generated).
// GeneratedCount/Total drive the polling page's progress bar.
...
// GenerateTriviaInput is the single-shot batch input. The workflow
// asks the agent for N q/a/e pairs in one call; the activity dedupes
// against the deck's existing prompts before returning.
